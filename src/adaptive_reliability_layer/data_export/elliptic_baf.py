...
def export_elliptic_from_raw(
    *,
    ...
    elliptic_root = raw_dir or (fraud_data_dir(root=root) / "raw" / "elliptic")
    features_path = elliptic_root / "elliptic_txs_features.csv"
    classes_path = elliptic_root / "elliptic_txs_classes.csv"
    ...
    output = fraud_data_dir(root=root) / output_name
...
def export_baf_from_raw(
    *,
    ...
    baf_root = raw_dir or (fraud_data_dir(root=root) / "raw" / "baf")
    source = _resolve_baf_variant_csv(baf_root, variant=variant)
    if source is None:
        return None
...
def export_elliptic_synthetic_fallback(*, rows: int = 80_000, seed: int = 23, root: Path | None = None) -> Path:
    rng = np.random.default_rng(seed)
    ...
    for index in range(scaled.shape[1]):
        out[f"feature_{index}"] = scaled[:, index]
    output = fraud_data_dir(root=root) / "elliptic_fraud.csv"
    pd.DataFrame(out).to_csv(output, index=False)
    return output


def export_baf_synthetic_fallback(*, rows: int = 120_000, seed: int = 29, root: Path | None = None) -> Path:
    rng = np.random.default_rng(seed)
    ...
    for index in range(scaled.shape[1]):
        out[f"feature_{index}"] = scaled[:, index]
    output = fraud_data_dir(root=root) / "baf_base_fraud.csv"
    pd.DataFrame(out).to_csv(output, index=False)
    return output
