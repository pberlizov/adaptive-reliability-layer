...
def export_paysim_synthetic(*, rows: int = 28000, seed: int = 7, root: Path | None = None) -> Path:
    ...
    frame = {"time_rank": steps.astype(np.int64), "label": labels}
    for index in range(scaled.shape[1]):
        frame[f"feature_{index}"] = scaled[:, index]
    output = fraud_data_dir(root=root) / "paysim.csv"
    pd.DataFrame(frame).sort_values("time_rank").to_csv(output, index=False)
    return output


def export_ieee_from_raw(*, max_rows: int = 40000, seed: int = 11, root: Path | None = None) -> Path | None:
    raw = fraud_data_dir(root=root) / "raw" / "train_transaction.csv"
    if not raw.exists():
        return None
    ...
    output = fraud_data_dir(root=root) / "ieee_cis_sample.csv"
...
def export_ieee_full_from_raw(
    *,
    ...
    fraud_dir = fraud_data_dir(root=root)
...
def export_ieee_synthetic(*, rows: int = 32000, seed: int = 11, root: Path | None = None) -> Path:
    rng = np.random.default_rng(seed)
